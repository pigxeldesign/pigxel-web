...
const AllCategoriesPage: React.FC = () => {
  ...
  const statusOptions = ['All', 'Featured', 'New', 'Popular'];
  ...
  useEffect(() => {
    filterAndSortDApps();
  }, [dapps, searchTerm, selectedCategories, selectedBlockchains, selectedStatus, sortBy]);
  ...
  const filterAndSortDApps = () => {
    ...
    if (selectedStatus && selectedStatus !== 'All') {
      switch (selectedStatus) {
        case 'Featured':
          filtered = filtered.filter(dapp => dapp.is_featured);
          break;
        case 'New':
          filtered = filtered.filter(dapp => dapp.is_new);
          break;
        case 'Popular':
          filtered = filtered.filter(dapp => dapp.rating && dapp.rating >= 4.5);
          break;
      }
    }
  ...
  const hasActiveFilters = searchTerm || selectedCategories.length > 0 || selectedBlockchains.length > 0 || selectedStatus;
    ...
    return (
      <div className="pt-16 min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400">Loading all dApps...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="pt-16 min-h-screen flex items-center justify-center">
        <div className="text-center px-4">
          ...
          <h1 className="text-2xl font-bold text-white mb-4">Error Loading dApps</h1>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <div className="flex flex-col lg:flex-row gap-4 mb-6">
              <div className="flex-1 relative">
                ...
                <input
                  type="text"
                  placeholder="Search dApps by name, description, or category..."
                  value={searchTerm}
                ...
                >
                  ...
                  {hasActiveFilters && (
                    <span className="ml-2 bg-white text-purple-600 text-xs px-2 py-1 rounded-full">
                      {selectedCategories.length + selectedBlockchains.length + (selectedStatus ? 1 : 0)}
                    </span>
                  )}
                        ...
                        {statusOptions.map((status) => (
                          ...
                          >
                            <input
                              type="radio"
                              name="status"
                              checked={selectedStatus === status || (status === 'All' && !selectedStatus)}
                              onChange={() => setSelectedStatus(status === 'All' ? '' : status)}
                              className="w-4 h-4 text-purple-600 bg-gray-700 border-gray-600 focus:ring-purple-500 mr-3"
                            />
            ...
            {hasActiveFilters && (
              <div className="flex flex-wrap gap-2 mb-4">
                ...
                {selectedStatus && (
                  <span className="flex items-center px-3 py-1 bg-green-600/20 text-green-300 rounded-full text-sm">
                    {selectedStatus}
                    <button
            ...
            <div className="flex items-center justify-between text-sm text-gray-400 mb-6">
              <span>
                Showing {Math.min(startIndex + 1, filteredDApps.length)}-{Math.min(endIndex, filteredDApps.length)} of {filteredDApps.length} dApps
              </span>
              {totalPages > 1 && (
          ...
          {filteredDApps.length === 0 ? (
            ...
            >
              ...
              <h3 className="text-xl font-bold text-white mb-2">No dApps found</h3>
