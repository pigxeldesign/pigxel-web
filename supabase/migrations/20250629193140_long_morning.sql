...
-- Update all other SECURITY DEFINER functions to have search_path set
-- We'll take a simple approach of just listing the known functions that need fixing
DO $$
BEGIN
  -- Check if update_updated_at_column exists and fix it
  IF EXISTS (
    SELECT 1 FROM pg_proc 
  ...
  -- Check if update_flow_screen_count exists and fix it
  IF EXISTS (
    SELECT 1 FROM pg_proc 
  ...
  -- Check if create_profile_for_new_user exists and fix it
  IF EXISTS (
    SELECT 1 FROM pg_proc 
  ...
  -- Check if update_newsletter_updated_at_column exists and fix it
  IF EXISTS (
    SELECT 1 FROM pg_proc 
...
DO $$
BEGIN
  -- Grant execution permissions to the appropriate functions
  ...
  -- If is_premium_user exists, grant execute permissions
  IF EXISTS (
    SELECT 1 FROM pg_proc 
    WHERE proname = 'is_premium_user'
    AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
