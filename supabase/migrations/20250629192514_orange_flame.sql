...
1. Security Fixes
   - Sets secure search_path in all SECURITY DEFINER functions
   - Fixes potential vulnerability where functions could be tricked into executing malicious code

2. Functions Modified
   - All existing functions marked as SECURITY DEFINER
   - Ensures proper security by restricting function lookup to pg_catalog and public

3. Technical Details
   - Sets search_path to pg_catalog, public
...
-- Fix search_path in update_updated_at_column function (if it exists)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_proc 
...
-- Fix search_path in update_flow_screen_count function (if it exists)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_proc 
    WHERE proname = 'update_flow_screen_count'
    AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
  ) THEN
    EXECUTE $EXEC$
...
-- Fix search_path in create_profile_for_new_user function (if it exists)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_proc 
...
-- Fix any other SECURITY DEFINER functions not explicitly mentioned
DO $$
DECLARE
  func_record RECORD;
BEGIN
  ...
  LOOP
    -- Check if function_def already contains SET search_path
    IF func_record.function_def NOT ILIKE '%SET search_path = pg_catalog, public%' THEN
      -- Get the function definition components
      DECLARE
        func_name text := func_record.function_name;
        func_def text := func_record.function_def;
        begin_pos integer;
        returns_pos integer;
        lang_pos integer;
        params_part text;
        ...
        body_part text;
        lang_part text;
        security_part text := 'SECURITY DEFINER';
      BEGIN
        -- Extract parts of the function definition
        begin_pos := position('BEGIN' IN upper(func_def));
        returns_pos := position('RETURNS ' IN upper(func_def));
        lang_pos := position('LANGUAGE ' IN upper(func_def));
        
        IF begin_pos > 0 THEN
          -- Add SET search_path after BEGIN
          EXECUTE format(
            'CREATE OR REPLACE FUNCTION %s %s LANGUAGE %s',
            substring(func_def from 1 for begin_pos + 5),
            E'\n  SET search_path = pg_catalog, public;\n  ' || 
            substring(func_def from begin_pos + 6 for (lang_pos - begin_pos - 6)),
            substring(func_def from lang_pos)
          );
...
BEGIN
  ...
  EXECUTE 'REVOKE ALL ON FUNCTION admin_save_dapp(jsonb, text) FROM PUBLIC;';
  EXECUTE 'GRANT EXECUTE ON FUNCTION admin_save_dapp(jsonb, text) TO authenticated;';
  
  EXECUTE 'REVOKE ALL ON FUNCTION direct_insert_dapp(jsonb) FROM PUBLIC;';
  EXECUTE 'GRANT EXECUTE ON FUNCTION direct_insert_dapp(jsonb) TO authenticated;';
  
  EXECUTE 'REVOKE ALL ON FUNCTION simple_insert_dapp(jsonb) FROM PUBLIC;';
  EXECUTE 'GRANT EXECUTE ON FUNCTION simple_insert_dapp(jsonb) TO authenticated;';
  
  EXECUTE 'GRANT EXECUTE ON FUNCTION check_auth_status() TO authenticated;';
  EXECUTE 'GRANT EXECUTE ON FUNCTION check_auth_status() TO anon;';
