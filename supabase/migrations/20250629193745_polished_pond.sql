...
This migration addresses the "Function Search Path Mutable" security issue by ensuring
all SECURITY DEFINER functions explicitly set their search_path to pg_catalog, public.
...
-- Check if is_premium_user exists and fix its search_path
SELECT EXISTS (
  SELECT 1 FROM pg_proc 
  WHERE proname = 'is_premium_user'
  AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
) INTO TEMPORARY is_premium_user_exists;

-- Drop and recreate is_premium_user if it exists
DROP FUNCTION IF EXISTS is_premium_user();

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_temp.is_premium_user_exists WHERE exists = true) THEN
    EXECUTE '
...
-- Check for update_flow_screen_count function
DROP FUNCTION IF EXISTS update_flow_screen_count() CASCADE;
...
-- Check for create_profile_for_new_user function
...
-- Check for update_newsletter_updated_at_column function
DROP FUNCTION IF EXISTS update_newsletter_updated_at_column() CASCADE;
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_proc 
    WHERE proname = 'update_newsletter_updated_at_column'
    AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
  ) THEN
    EXECUTE '
...
REVOKE ALL ON FUNCTION direct_insert_dapp(jsonb) FROM PUBLIC;
...
-- Grant is_premium_user if it exists
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_proc 
    ...
    EXECUTE 'GRANT EXECUTE ON FUNCTION is_premium_user() TO authenticated';
  END IF;
END $$;