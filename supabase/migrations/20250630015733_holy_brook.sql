/*
  # Fix function search paths and permissions

  1. Changes
     - Fix search_path in security definer functions to ensure proper schema resolution
     - Update is_premium_user without dropping it (to avoid breaking policy dependencies)
     - Ensure proper permissions for all functions
     - Add debugging capabilities to database functions
*/
...
-- Update is_premium_user without dropping it (preserve policy dependencies)
...
BEGIN
  IF EXISTS (
    ...
    -- Update the existing function to fix the search_path
    EXECUTE '
