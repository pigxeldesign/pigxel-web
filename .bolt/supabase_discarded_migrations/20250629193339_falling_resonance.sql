/*
  ...
  1. Security Improvements
     - Adds explicit `SET search_path = pg_catalog, public` to all security definer functions
     - Prevents SQL injection attacks via search path manipulation
  
  2. Updated Functions
     - is_admin()
     - is_premium_user() (if it exists)
     - admin_save_dapp()
     - direct_insert_dapp()
     - simple_insert_dapp()
     - check_auth_status()
     ...
     - log_operation()
     - log_dapp_operation_with_debug()
     - Several system trigger functions

  3. Permissions
     - Updates function execution permissions for proper access control
*/
...
BEGIN
  ...
  IF func_exists THEN
    DROP FUNCTION IF EXISTS update_flow_screen_count();
    
    CREATE FUNCTION update_flow_screen_count()
    RETURNS TRIGGER AS $$
    DECLARE
      flow_id uuid;
      new_count integer;
...
-- Fix search_path in update_newsletter_updated_at_column function
DO $$
DECLARE
  func_exists boolean;
BEGIN
  ...
  IF func_exists THEN
    DROP FUNCTION IF EXISTS update_newsletter_updated_at_column();
    
    CREATE FUNCTION update_newsletter_updated_at_column()
    RETURNS TRIGGER AS $$
    BEGIN
      SET search_path = pg_catalog, public;
      NEW.updated_at = now();
